...
test("搜索引擎数据结构有效", () => {
  ...
  for (const engine of searchEngines) {
    ...
    assert.equal(engine.priority >= 1 && engine.priority <= 99, true, `搜索引擎优先级必须在 1-99: ${engine.id}`);
...
test("本地管理校验会拒绝错误链接、错误日期、空正文和错误模板", () => {
  ...
  const duplicatePriorityEngines = structuredClone(searchEngines);
  duplicatePriorityEngines[0].priority = duplicatePriorityEngines[1].priority;
  assert.throws(() => validateSearchEnginesPayload(duplicatePriorityEngines), /优先级重复/);
});
